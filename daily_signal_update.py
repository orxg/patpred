# -*- coding: utf-8 -*-
"""
Created on Wed Sep 12 16:58:58 2018
...
model_codes = [each.split('.')[0] for each in model_files]
#model_codes = pd.read_excel('models_daily.xlsx',dtype = {'model_code':str})
#model_codes = model_codes['model_code'].tolist()

today = dt.datetime.today()
today_str = today.strftime('%Y%m%d')
trade_calendar = get_trade_calendar('20100101',(today + dt.timedelta(days = 30)).strftime('%Y%m%d'))
...
feature_start_date = (dt.datetime.strptime(last_target_date,'%Y%m%d') - dt.timedelta(days = 60 * 5)).strftime('%Y%m%d')

stock_universe = ["'%s'"%each for each in model_codes]
...
features = get_data(SQL_GET_FEATURES.format(start_date = feature_start_date,
                                   ...
                                   stock_universe = stock_universe),'gb')
...
save_into_db(signals,'LDH_pattern_pred_daily_source',dtype_dict,'gb','replace')
SQL_MERGE_SIGNALS = '''
MERGE INTO LDH_pattern_pred_daily as T
USING LDH_pattern_pred_daily_source as S
